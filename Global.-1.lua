...
auto draw conflict lines (option on the thing) - WIP
auto not hitler (option on the thing) - WIP
...
4.1:
scripting zones size fix
gives pres and shot ignored on "claim"
log draws - get with "getDraws"
removed "text" save (they are all constants)
no player in inspect seat bug - now doesnt deal
says disable security even if disabled (still needs to be reloaded to update, but says correct thing on settings enter)
claim system only counts if it fits the type given
...
function settingsPannelMakeButtons()
	...
	if settingsPannel then
		...
		makeSquareButtonLabel(settingsPannel, options.richardRule, check_string, '', 'Handsome Richard Rule', 'richardFlip', {startX, 0.2, startZ + offsetZ * 14}, 6, true)
...
function startVoteCheck()
	...
	if (#voteCards == getNumAlivePlayers() -1 and options.richardRule == true) then
		...
		if fiveSeconds > 0 then
			broadcastToAll('Voting in: ' .. fiveSeconds, stringColorToRGB('Green'))
			voteDone = false
		--elseif fiveSeconds == 0 then
			--bolRichard = true
		end
		if fiveSeconds == 0 then bolRichard = true end
	end
		...
		for i, color in pairs(colours) do
			if (roles[color] ~= nil) then
				if (roles[color] == 'hitler') then
					...
					hitCard.setRotation(getObjectFromGUID(policySafety_zone_guids[color]).getRotation())
				elseif (roles[color] == 'fascist') then
					...
					fasCards[randomCard].setRotation(getObjectFromGUID(policySafety_zone_guids[color]).getRotation())
				...
				elseif (roles[color] == 'liberal') then
					...
					libCards[randomCard].setRotation(getObjectFromGUID(policySafety_zone_guids[color]).getRotation())
